...
-- mod entry chunk runs. Supported 0.1.x releases expect one flat option list.
...
local optionSchema = {
  ...
  { key = "autoCatch", label = "AUTO CATCH", type = "toggle", default = false,
    visibleIf = when("autoBattle", true) },
  { key = "autoCatchNewOnly", label = "CATCH NEW ONLY", type = "toggle",
    default = false, visibleIf = whenAll(
      when("autoBattle", true), when("autoCatch", true)) },
  ...
  { key = "autoCatchBall", label = "AUTO CATCH BALL", type = "choice",
    ...
    }, default = "best", visibleIf = whenAll(
      when("autoBattle", true), when("autoCatch", true)) },
  { key = "showBallCounts", label = "SHOW BALL COUNTS", type = "toggle",
    default = true, visibleIf = whenAll(
      when("autoBattle", true), when("autoCatch", true)) },
  ...
  { key = "fieldAbilitiesAuto", label = "AUTO FIELD MOVES", type = "toggle",
    default = false, visibleIf = when("fieldAbilities", true) },
  ...
  { key = "partyOverviewCorner", label = "PARTY CORNER", type = "choice",
    ...
    }, default = "bottom-left", visibleIf = when("partyOverview", true) },
  { key = "partyOrder", label = "PARTY ORDER", type = "choice",
    choices = {
      ...
      { "LEVEL HIGH", "level" },
      { "TYPE ORDER", "type" },
    }, default = "native", visibleIf = when("partyOverview", true) },
  { key = "autoPartyOrder", label = "AUTO PARTY ORDER", type = "toggle",
    default = false, visibleIf = when("partyOverview", true) },
  ...
  { key = "minimapBorder", label = "MAP BORDER", type = "choice",
    choices = {
      ...
      { "SQUARE", "square" },
      { "OFF", "off" },
    }, default = "rounded", visibleIf = when("minimap", true) },
  ...
  { key = "encounterTrackerCorner", label = "TRACKER CORNER", type = "choice",
    choices = {
      ...
      { "BOTTOM LEFT", "bottom-left" },
      { "BOTTOM RIGHT", "bottom-right" },
    }, default = "top-right", visibleIf = when("encounterTracker", true) },
  { key = "encounterTrackerTracked", label = "SHOW TRACKED", type = "toggle",
    default = true, visibleIf = when("encounterTracker", true) },
  ...
  { key = "autoRenameTrades", label = "AUTO RENAME TRADES", type = "toggle",
    default = false, visibleIf = when("renameNpcTrades", true) },
  ...
  { key = "restoreName", label = "RESET NAME", type = "toggle", default = true,
    visibleIf = when("renameAnywhere", true) },
}
...
if isGen1 then
  ...
  for _, row in ipairs({
    ...
    row.visibleIf = when("encounterTracker", true)
